...
def profile(request):
    ...
    context = {'title': 'Пофиль', # если есть одинаковые поля у юзера и формы, то они заполняться даннными
        'form': UserProfileForm(instance=request.user), 'basket': Basket.objects.filter(user=request.user), }
